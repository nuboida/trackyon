...
import { EditTaskDetailRequest, MemoTaskDetailRequest } from '@modules/call-memo/models/call-memo-response.model';
...
@UntilDestroy()
@Component({
  selector: 'olla-dept-memo-edit',
  templateUrl: './dept-memo-edit.component.html',
  styleUrls: ['./dept-memo-edit.component.scss']
})
export class DeptMemoEditComponent implements OnInit {
  ...
  editMode = false;
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: {taskId: number, taskName: string, deptId: number, taskDetailId: string, taskDate: string, taskNotes: string, taskScore: number, taskComment: number},
    public dialogRef: MatDialogRef<DeptMemoEditComponent>,
