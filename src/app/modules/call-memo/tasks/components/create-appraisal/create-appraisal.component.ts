...
@UntilDestroy()
@Component({
  selector: 'olla-create-appraisal',
  templateUrl: './create-appraisal.component.html',
  styleUrls: ['./create-appraisal.component.scss']
})
export class CreateAppraisalComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: {
      details: MemoTaskStaffDetailsResponse,
      note: boolean,
      appraise: boolean,
      weight: number,
      staffId: string,
  ...
  handleDelete(): void {
    this.loading = true;
    this.callMemoService.deleteStaffAppraisal(this.data.details.id).pipe(untilDestroyed(this))
      .subscribe(
        () => {
